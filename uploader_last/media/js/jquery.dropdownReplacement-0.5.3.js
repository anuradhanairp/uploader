 ...
 *
 * Works with jQuery 1.4.x, 1.5.x, 1.6.0
...
;(function($){
$.dropdownReplacement = {defaults:{ //version 0.5.3
	...
	selectClass: "id_filter", //user defined class to give to the element that will act as the <select>
...
var dropdownReplacement = function(opts, operator){
  if(typeof opts === "string" && opts === "option"){
	...
	var textList = [];
	var textToIndex = {};//given text, gives you the index of that text in the options list;
  ...
  var detectCharWidth = function(testText){
	...
	var constructOptions = function($select){ //this is done once: order #1
		...
		for(var i = 0; i < l.length; i++){
			textToIndex[l[i].t] = textList.length;
	...
	var arrowMove =	function(direction){ //direction is "prev" or "next"
		...
		if(direction !== "first" && direction !== "last"){
			if($selectedOption.length > 0) {
				$newSelected = $selectedOption[direction]("a"); //this is more reliable way of locating the next element. 
				if($newSelected.length == 0){ //is user uses a custom dropdown body, then next/prev might not return an option, fall back approach:
					var idx = textToIndex[$selectedOption.text()]+(direction==="next"?1:-1);
					$newSelected = $(textToOption[textList[idx]]);
				}
			} else {
